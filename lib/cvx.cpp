...
void headposetracker::cvx::getROI(const cv::Mat& src, cv::Mat& dst, const cv::Rect& roi, int borderType)
{
...
float headposetracker::cvx::histKmeans(const cv::Mat_<float>& hist, int bin_min, int bin_max, int K, float init_centres[], cv::Mat_<uchar>& labels, cv::TermCriteria termCriteria)
{
...
cv::RotatedRect headposetracker::cvx::fitEllipse(const cv::Moments& m)
{
    cv::RotatedRect ret;
...
cv::Vec2f headposetracker::cvx::majorAxis(const cv::RotatedRect& ellipse)
{
